...
# Function to find the least squares fit for a line
def least_squares_fit(X, y):
    X = np.column_stack((X, np.ones_like(X)))  # Add a column of ones to the X matrix
    coeff = np.linalg.lstsq(X, y, rcond=None)[0]  # Find the coefficient vector
...
def generate_data(num_points):
    ...
    y = 2 * X + 1 + np.random.randn(num_points)  # Add noise
